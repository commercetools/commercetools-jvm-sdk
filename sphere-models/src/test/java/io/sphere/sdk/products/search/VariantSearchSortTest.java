...
public class VariantSearchSortTest {
    ...
    public void buildsAscendingSortExpression() throws Exception {
        assertThat(attributeModel().sorted().byAsc().toSphereSort()).isEqualTo("variants.attributes.size asc");
    }

    @Test
    public void buildsDescendingSortExpression() throws Exception {
        assertThat(attributeModel().sorted().byDesc().toSphereSort()).isEqualTo("variants.attributes.size desc");
    }

    @Test
    public void buildsAscendingSortExpressionWithAppendedParameter() throws Exception {
        assertThat(attributeModel().sorted().byAscWithMax().toSphereSort()).isEqualTo("variants.attributes.size asc.max");
    }

    @Test
    public void buildsDescendingSortExpressionWithAppendedParameter() throws Exception {
        assertThat(attributeModel().sorted().byDescWithMin().toSphereSort()).isEqualTo("variants.attributes.size desc.min");
    }
