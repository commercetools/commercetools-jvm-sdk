...
````scala
libraryDependencies += "io.sphere" %% "sphere-play-sdk" % "0.51.0" withSources()
````

Play 2.2.x: (exclude to prevent that two play jars in the classpath)

````scala
libraryDependencies += "io.sphere" %% "sphere-play-sdk" % "0.51.0" withSources() exclude("play", "play_2.10")
````
