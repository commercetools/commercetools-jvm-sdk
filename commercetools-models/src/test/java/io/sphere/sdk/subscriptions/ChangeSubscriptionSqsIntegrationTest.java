...
public class ChangeSubscriptionSqsIntegrationTest extends IntegrationTest {
    ...
    public void productCreated() throws Exception {
        final SqsDestination sqsDestination = getSqsDestination(subscription);
        final String queueUrl = sqsDestination.getQueueUrl().toString();

        withProduct(client(), product -> {
            final ReceiveMessageResult result = AmazonSQSClientBuilder.defaultClient().receiveMessage(queueUrl);
            ...
            final ResourceCreatedPayload resourceCreatedPayload =
    ...
    public static void setup() throws Exception {
        ...
        final SubscriptionDraftDsl subscriptionDraft = withProductChanges(sqsSubscriptionDraftBuilder()).build();
    ...
    private static SqsDestination getSqsDestination(final Subscription subscription) {
        final Destination destination = subscription.getDestination();
        assertThat(destination).isInstanceOf(SqsDestination.class);

        return (SqsDestination) destination;
    }
}
