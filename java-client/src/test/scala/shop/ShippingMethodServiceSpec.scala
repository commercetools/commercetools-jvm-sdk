...
class ShippingMethodServiceSpec extends WordSpec with MustMatchers {
  ...
  "Get shipping methods for location" in {
    val eur = Currency.getInstance("EUR") 
    val req = MockSphereClient.create(shippingMethodsResponse = FakeResponse("[]")).shippingMethods().forLocation(new Location(CountryCode.DE), eur)
  ...
  "Get shipping methods for cart" in {
    val req = MockSphereClient.create(shippingMethodsResponse = FakeResponse("[]")).shippingMethods().forCart("cart1")
    URLDecoder.decode(req.asInstanceOf[FetchRequestImpl[List[ShippingMethod]]].getRequestHolder.getUrl, "UTF-8") must be (
