...
class HttpSphereRequestExecutorSpec extends WordSpec with ShouldMatchers {
  ...
  val executor = new HttpSphereRequestExecutor(new HttpClientTestDouble {
    override def testDouble[T](requestable: Requestable[T]): HttpResponse = null
  }, config)

  "HttpSphereRequestExecutor" must {
    s"convert valid fetch request into successful $ClassName" in {
      val result: SphereResultRaw[Xyz] = executor.requestToSphereResult(HttpResponse.of(200, """{"id":1}"""), new XyzService().fetchById("1"))
      result should be(SphereResultRaw.success(new Xyz("1")))
    }

    s"convert not found into error $ClassName" in {
      val result: SphereResultRaw[Xyz] = executor.requestToSphereResult(HttpResponse.of(404, """{"statusCode":404, "message":"not found"}"""), new XyzService().fetchById("1"))
