...
public class CustomProbeRunner extends BlockJUnit4ClassRunner {

private static final Logger LOG = LoggerFactory.getLogger(CustomProbeRunner.class);
...
public CustomProbeRunner(Class<?> klass,Class<?> testSuiteClass) throws InitializationError {
    super(klass);
    LOG.info("creating PaxExam runner for {}", klass);
    try {
        Object testClassInstance = klass.newInstance();
        manager = ReactorManager.getInstance();
        ExamReactor examReactor = manager.prepareReactor(testSuiteClass, testSuiteClass.newInstance());
        addTestsToReactor(examReactor, klass, testClassInstance);
        //set to perMethod or perClass, PerSuite
        stagedReactor = examReactor.stage(getExamReactorStrategy(testSuiteClass));
    }
...
private static StagedExamReactorFactory getExamReactorStrategy(Class<?> testSuiteClass){
       try {
           ExamReactorStrategy strategy = testSuiteClass.getAnnotation(ExamReactorStrategy.class);
